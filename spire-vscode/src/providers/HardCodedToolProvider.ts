...
/**
 * Day 0 Provider: Hard-coded keyword matching.
 *
 * Maps natural-language user prompts to graph-memory MCP tool calls using
 * keyword patterns and basic entity extraction.
 *
 * WHY THIS EXISTS:
 ...
 */
export class HardCodedToolProvider implements ToolCallProvider {
  /**
  ...
  analyzePrompt(prompt: string): ProviderDecision {
    console.log(`[HardCodedProvider] Analyzing: "${prompt}"`);
    ...
    console.log(
      `[HardCodedProvider] → ${decision.toolName || 'no tool'}, Confidence: ${confidence.toFixed(2)}`
    );
    return decision;
  }
  ...
  getProviderInfo(): ProviderInfo {
    return {
      name: 'HardCodedToolProvider',
      version: '1.0.0',
      description: 'Rule-based tool selection using keyword matching with Day 0 functionality',
  ...
  private buildArguments(
    ...
    switch (toolName) {
      ...
      case 'graph-memory__get_all_nodes':
        const typeMatch = prompt.match(/(?:all|list)\s+([a-zA-Z_]+)/i);
